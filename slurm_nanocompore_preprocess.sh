...
# align to reference
minimap2 -ax map-ont -L ${2} ${3} | samtools view -bh -F 2324 -q 10 | samtools sort -O bam > $3/aligned.bam

# build index
samtools index $3/aligned.bam
...
nanopolish eventalign --threads 10 --reads ${3} --bam $3/aligned.bam --genome ${2} --samples --print-read-names --scale-events --samples > ${3}/eventalign_reads.tsv

# data has to be collapsed per kmer and indexed by NanopolishComp Eventalign_collapse
NanopolishComp Eventalign_collapse -i ${3}/eventalign_reads.tsv -o ${3}/eventalign_collapsed_reads.tsv
